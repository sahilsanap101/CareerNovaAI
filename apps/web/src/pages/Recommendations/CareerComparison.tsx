        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {comparisonData?.map((item) => (
              <Card key={item.careerPath.id} className="border-t-4 border-t-primary-500">
                <CardHeader>
                  <CardTitle>{item.careerPath.name}</CardTitle>
                  <CardDescription>{item.careerPath.category}</CardDescription>
                </CardHeader>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="text-xs space-y-1.5">
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold text-slate-800 dark:text-slate-200">{item.careerPath.averageSalary}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-500">Growth Rate:</span>
                      <span className="font-semibold text-green-600">{item.careerPath.growthRate}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-500">Demand Level:</span>
                      <Badge variant="default">{item.careerPath.demandLevel}</Badge>
                    </div>
                  </div>
                  ...
                  <div className="pt-3 border-t border-slate-200 dark:border-slate-800 space-y-2">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {item.careerPath.requiredSkills?.map((req) => (
